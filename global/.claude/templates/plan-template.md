...
├── figma-nodes.md      # UI 任务专属：Figma 节点映射表（/plan 阶段生成）
...
<!-- 仅 UI 任务填写此项，通过 Figma MCP 扫描设计后生成 -->

**Figma 文件**：[文件名称] | **文件 Key**：[key] | **节点**：[根节点 ID]

**设计组件清单**：

| 组件名 | 职责 | 节点 ID | 关键样式说明 |
|--------|------|---------|-------------|
| [例：ReportCard] | [报告卡片容器] | `1525:25952` | [rounded-12px, p-24px] |
| [例：Header] | [标题行] | `1525:25953` | [flex, gap-8px] |

**Design Token 引用**：

- [记录 Figma 中的设计变量名称，如 `--te-color-primary`]

**说明**：详细节点映射见 `figma-nodes.md`，实现时按节点 ID 调用 `get_design_context` 获取视觉细节。
