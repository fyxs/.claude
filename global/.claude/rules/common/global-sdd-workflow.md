...
> 本规则定义了 Spec-Driven Development (SDD) 工作流的触发条件和执行流程。
...
SDD 是**规格层**，`global-development-workflow` 是**实现层**，两者串联使用：
...
SDD 规格层
  ...
  /tasks   → tasks.md（任务列表）
       ↓
global-development-workflow 实现层
  研究与复用 → Annotation Cycle → TDD → 代码审查 → 验证 → Git 提交
```

- `/tasks` 输出的 `tasks.md` 直接作为 `global-development-workflow` 实现阶段的输入
- SDD 的 `research.md` 对应 `global-development-workflow` 的研究产物持久化要求
- SDD 的 `plan.md` 对应 `global-development-workflow` 的 Annotation Cycle 文件
- TDD 要求在两个规则中一致：Article III（NON-NEGOTIABLE）
